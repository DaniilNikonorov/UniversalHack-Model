...
def data_tranform_to_recbole(interactions_name, users_name,
    ...
    if not(os.path.isfile(f'content/{save_dataset_name}/{save_dataset_name}.inter')):
        ...
        interactions_df.to_csv(f'content/{save_dataset_name}/{save_dataset_name}.inter', sep='\t',
                               index=False, header=['user_id:token', 'item_id:token',
                                                    'rating:float', 'timestamp:float'], float_format='%d')

    if not(os.path.isfile(f'content/{save_dataset_name}/{save_dataset_name}.user')):
        client = clickhouse_connect.get_client(host='94.139.255.66', username='danya', password='qwerty')
        users = client.query(f"SELECT * FROM {users_name}")
        users_df = pd.DataFrame(data = users.result_rows, columns = users.column_names)
        users_df.to_csv(f'content/{save_dataset_name}/{save_dataset_name}.user', sep='\t', index=False, header=['user_id:token', 'age:token', 'income:token',
                                        'sex:token', 'kids_flg:token'], float_format='%d')

    if not (os.path.isfile(f'content/{save_dataset_name}/{save_dataset_name}.item')):
        client = clickhouse_connect.get_client(host='94.139.255.66', username='danya', password='qwerty')

        items = client.query(f"SELECT * FROM {items_names}")
        items_df = pd.DataFrame(data = items.result_rows, columns = items.column_names)
        items_df.to_csv(f'content/{save_dataset_name}/{save_dataset_name}.item', sep='\t', index=False, header=['item_id:token', 'content_type:token', \
